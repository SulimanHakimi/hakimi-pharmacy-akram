...
import { Invoice, Drug, Return, getSettings } from '@/lib/models';
...
export const GET = route(async () => {
  ...
  const [todayInvs, todayReturns, drugs] = await Promise.all([
    Invoice.find({ date: { $gte: from } }),
    Return.find({ date: { $gte: from } }),
    Drug.find()
  ]);
  ...
  const t = totals(todayInvs, todayReturns);

  return ok({
    sales: t.rev, invoiceCount: t.invs, profit: t.profit, returned: t.returned,
